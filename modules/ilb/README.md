...
|------|-------------|------|---------|:--------:|
| `backends` | List of backends, should be a map of key-value pairs for each backend, must have the 'group' key. | `list(object({ group = string description = optional(string) failover = optional(bool) }))` | n/a | Yes |
| `health_check` | Health check to determine whether instances are responsive and able to do work | `object({ type = string check_interval_sec = optional(number) healthy_threshold = optional(number) timeout_sec = optional(number) unhealthy_threshold = optional(number) response = optional(string) proxy_header = optional(string) port = optional(number, 80) port_name = optional(string) request = optional(string) request_path = optional(string) host = optional(string) enable_log = optional(bool, false) })` | n/a | Yes |
...
| `subnets` | Optional: A map containing subnet details Used to derive the subnetwork URI if subnetwork is not provided. | `list(object({ id = string region = string purpose = string ip_cidr_range = string }))` | `[]` | No |
