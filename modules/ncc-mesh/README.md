...
| `auto_accept_projects` | n/a | `list(string)` | `[]` | No |
| `vpc_spokes` | VPC network that is associated with the spoke. link_producer_vpc_network: Producer VPC network that is peered with vpc network. In a mesh hub all spokes reach each other any-to-any, so no group assignment is needed. | `map(object({` | n/a | Yes |
