...
resource "google_compute_region_instance_group_manager" "mig" {
  ...
  # Flexible MIG: let the group create VMs from a set of machine types instead of
  # a single fixed shape. Compute Engine picks per instance_selections rank/availability.
  ...
  distribution_policy_target_shape = var.distribution_policy_target_shape != null ? var.distribution_policy_target_shape : (length(var.instance_selections) > 0 ? "ANY" : null)
  distribution_policy_zones        = local.distribution_policy_zones
  dynamic "update_policy" {
    for_each = length(var.update_policy) > 0 ? var.update_policy : (length(var.instance_selections) > 0 ? [{
      type                           = "OPPORTUNISTIC"
