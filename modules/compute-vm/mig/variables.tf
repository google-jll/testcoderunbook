...
variable "standby_policy" {
  description = "Standby policy for a flexible MIG. Used with target_suspended_size / target_stopped_size to keep a pool of pre-created stopped/suspended VMs for faster scale-out. mode is MANUAL or SCALE_OUT_POOL."
  type = object({
...
variable "target_suspended_size" {
  description = "Number of instances to keep suspended. Requires standby_policy."
  type        = number
  default     = null
}

variable "target_stopped_size" {
  description = "Number of instances to keep stopped. Requires standby_policy."
  type        = number
  default     = null
}
