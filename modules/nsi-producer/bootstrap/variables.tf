variable "name_prefix" {
  description = "Prefix of the name of Google Cloud Storage bucket, followed by 10 random characters. Ignored when bucket_name is set."
  default     = "paloaltonetworks-firewall-bootstrap-"
  type        = string
}

variable "bucket_name" {
  description = "Explicit, full bucket name. When set, the random suffix is NOT used (and no random_string is created) — use this to adopt/import an existing bootstrap bucket without renaming it."
  default     = null
  type        = string
}
