...
| `inspect_ranges` | IP ranges whose traffic to/from this consumer VPC is steered to the firewall. Defaults to all traffic. | `list(string)` | `["0.0.0.0/0"]` | No |
...
| `rules` | Optional map of custom network firewall policy rules (e.g. targeting secure tags) passed down to the network firewall policy. | `any` | `{}` | No |
## Outputs
